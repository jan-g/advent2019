...
parse width height ls = ls & head & chunksOf (width * height)

day8 ls =
  let width = 25
      height = 6
      layers = parse width height ls
      zeroes = foldl (\m l -> Map.insert (countDigits '0' l) l m) Map.empty layers
      (_, leastZeroes) = Map.findMin zeroes
      ones = countDigits '1' leastZeroes
      twos = countDigits '2' leastZeroes
  in ones * twos
  where
    countDigits d layer = layer & filter (== d) & length
...
day8b ls =
  let width = 25
      height = 6
      layers = parse width height ls
      folded = transpose layers
      final = map finalPixel folded
      rendered = final & map draw & chunksOf width
  in  unlines rendered
  where
    draw '0' = ' '
    draw '1' = '#'
    
finalPixel ps = dropWhile (== '2') ps & head