...
program, replace position 1 with the value 12 and replace position 2 with the value 2. What value is left at position
...
day2b ls =
  let prog = parse (head ls)
      outs = [(noun, verb, ans) | noun <- [0..99],
                                  verb <- [0..99],
                                  let ans = outp prog noun verb,
                                  ans == 19690720]
      (n, v, _) = head outs
  in n * 100 + v
  where
      outp prog n v = (run 0 (prog // [(1, n), (2, v)])) ! 0
